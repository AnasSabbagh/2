...
public class Book {
    ...
    static ArrayList<Book> booksList = new ArrayList();  // المكان اللي تتخزن فيه الكتي
    // القيم اللي ياخذها الكتاب
    private String name;
    ...
    // setters and getters methodes for all data.
    ...
    public static void displayBooks() {  // هذي المثيود عشان تعرض للمستخدم الكتب المخزنة                
        ArrayList<Book> listBook = Book.booksList;
       // اذا كان البوكليست "الاراي" فيها عناصر بيروح يطبع الاسم حق الكتاب والمؤلف والسعر غير كذا بتجي رسالة للمستخدم انه مافي ولا كتاب
        if (!listBook.isEmpty()) { 
            System.out.println("\n  ---- Books Storage ----");
    ...
    public static void addBooks(Scanner scan) { // هذي المثيود موجودة عشان تخليا لمستخدم يقدر يضيف كتب
        ...
        if (countBook < 0) {
            ...
            for (int i = 1; i <= countBook; i++) {
                ...
                String nameBook = scan.next();  //  ملاحظة انا خليت المستخدم يقدر يحط فقط كلمة وحدة لاني حاط نكست فقط
    ...
    public static boolean updateBook(Book newBook, Book oldBook) { // هذي الميثود موجودة عشان تستبدل بيانات الكتاب القديم بالبيانات الجديدة
        if (booksList.size() > 0) {
            int index = -1;
            for (int i = 0; i < booksList.size(); i++) {
                if (booksList.get(i).getName().equals(oldBook.getName())) {  // هذا اللوب بيشيك على الاندكس حق العناصر اللي في الاراي اذا الاسم اللي ادخله المستخدم موجود في احد الاندكس فا بيساويه في المتغير اندكس
                    index = i;
                    break;
                }
            }
    ...
    public static void updateBooks(Scanner scan) {
        boolean Menu = true;
        while (Menu) {
            System.out.println();
